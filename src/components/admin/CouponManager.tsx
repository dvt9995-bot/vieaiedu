...
interface C { code: string; percent_off: number; active: boolean; expires_at: string | null; max_uses: number | null; used_count: number; }

export default function CouponManager() {
  const [list, setList] = useState<C[]>([]);
  const [f, setF] = useState({ code: "", percent_off: "20", expires_at: "", max_uses: "" });
  ...
  async function add() {
    ...
    const r = await fetch("/api/admin/coupons", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: f.code, percent_off: Number(f.percent_off), expires_at: f.expires_at || null, max_uses: f.max_uses || null }) }).then((x) => x.json());
    if (r.ok) { toast(`Đã tạo mã ${f.code}`); setF({ code: "", percent_off: "20", expires_at: "", max_uses: "" }); load(); }
  ...
  return (
    <div className="max-w-[520px]">
      ...
      <div className="flex gap-2 flex-wrap mb-4">
        ...
        <input className={`${inp} w-28`} type="number" placeholder="Giới hạn lượt" title="Số lượt dùng tối đa (trống = không giới hạn)" value={f.max_uses} onChange={(e) => setF({ ...f, max_uses: e.target.value })} />
          ...
          <tbody>
            ...
            : list.map((c) => (
              <tr key={c.code} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-3 text-ink-3 text-xs whitespace-nowrap">{c.max_uses != null ? `${c.used_count ?? 0}/${c.max_uses} lượt` : `${c.used_count ?? 0} lượt`}</td>
