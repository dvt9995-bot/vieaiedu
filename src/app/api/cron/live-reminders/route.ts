...
export async function GET(req: Request) {
  ...
  // Lấy các buổi sắp diễn ra trong 70 phút tới
  const { data: sessions } = await admin.from("live_sessions")
    .select("id, course_id, title, starts_at, duration_min, reminded_1h, reminded_10m")
    .gte("starts_at", iso(now - 5 * 60000)).lte("starts_at", iso(now + 70 * 60000));

  for (const ss of sessions || []) {
    const start = new Date(ss.starts_at as string).getTime();
    const mins = (start - now) / 60000;
    let kind: "1h" | "10m" | null = null;
    ...
    const when = new Date(start).toLocaleString("vi-VN", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "2-digit", timeZone: "Asia/Ho_Chi_Minh" });
    const title = kind === "10m" ? "🔴 Lớp học bắt đầu sau 10 phút!" : "⏰ Lớp học bắt đầu sau 1 giờ";
    const body = `"${course.title}"${ss.title ? " — " + ss.title : ""} lúc ${when}. Bấm để vào lớp.`;
    for (const e of enr || []) {
      await notify({ userId: e.user_id as string, type: "learning", title, body, href: `/live/${course.slug}`, push: true });
      sent++;
    }
    await admin.from("live_sessions").update(kind === "10m" ? { reminded_10m: true } : { reminded_1h: true }).eq("id", ss.id);
  }
