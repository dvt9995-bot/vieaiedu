...
const FIELDS = ["title", "subtitle", "description", "category", "level", "price", "compare_price", "thumb", "instructor", "format", "capacity", "instructor_bio", "instructor_avatar", "faq", "guarantee", "result_images"];
...
export async function GET() {
  ...
  const { data } = await admin.from("courses").select("id, slug, title, subtitle, description, price, compare_price, status, review_status, review_note, total_minutes, thumb, category, level, instructor, format, capacity, instructor_bio, instructor_avatar, faq, guarantee, result_images").eq("owner_id", u.uid).order("created_at", { ascending: false });
